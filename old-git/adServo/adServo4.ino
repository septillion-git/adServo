/******************************************************************************
    adServo v4 - Use to control servo's with an adServo by Timo Engelgeer
   ...
   ******** v4 ************
   - Made function for button update
   - button update one button per call
   - Updated ServoControl (updates one servo per call)
   - Fixed program button pressed millis()
...
void loop(){
    ...
    */
    ...
    if(programButton.read() == LOW){
        ...
        else if(millis() - progButtonMillis > 3000){    //go into progMode if button is pressed for 3 sec
            progMode();
        }
    }
