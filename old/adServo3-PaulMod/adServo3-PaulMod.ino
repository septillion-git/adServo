/******************************************************************************
    adServo v3 PaulMod - Use to control servo's with an adServo by Timo Engelgeer
    Created by Timo Engelgeer (Septillion), May 4, 2015
    ...
    /////////Change settings, analog (non-static, non-oneButton//////////////////////
    ...
    /////////Analog mode with oneButton and static///////////////////////////////////
    Define ONE_BUTTON and STATIC_SERVO in "Vars and pinmaps" to use one button/pin per servo.
    Need to use both at the same time at the moment, no way to set a servo with ONE_BUTTON
    
    Set the position in the arrays ServoPos and ServoSpeed @ "Vars and pinmaps"
    
    NOTE: Program button is used to center. "Led" can be set to the onboard led (together with LedArduino)
   ...
   ******** PaulMod *******
   -added ONE_BUTTON and STATIC_SERVO modes
...
#if defined(STATIC_SERVO)
//Array for position for each servo {{Servo1_PosA, Servo1_PosB}, {Servo2_PosA, Servo2_PosB}, etc}
const byte ServoPos[][2] = {{0, 253}, {0, 253}, {0, 253}, {0, 253}};
//Array for position for each servo (0-253) {{Servo1_StepSize, Servo1_Interval}, {Servo2_StepSize, Servo2_Interval}, etc}
//stepSize every 20ms, higher is faster
//interval, 0 => 20ms, 1 => 40ms etc
const byte ServoSpeed[][2] = {{5, 0}, {5, 0}, {5, 0}, {5, 0}};
#endif

ServoControl servoControl;  //To control the servo's

const byte Led = 11;        //Led on the adServo
const byte LedArduino = LED_BUILTIN; //Led on the Arduino board
...
void setup(){
    ...
    for(byte i = 0; i < NrServos; i++){
        ...
        #else
        //Or load static
        servoControl.setEndPos(i, 0, ServoPos[i][0] * 4);    //set Position A (0-1012)
        servoControl.setEndPos(i, 1, ServoPos[i][1] * 4);    //set Position B (0-1012)
        servoControl.setSpeed(i, ServoSpeed[i][0], ServoSpeed[i][1]);  //set speed, interval every (0 + 1) * 20ms, a step of 5
        #endif
...
void loop(){
    ...
    for(byte i = 0; i < NrButtons; i++){
        ...
        //Normal mode/Two button mode
        #if !defined(ONE_BUTTON)
    ...
    //Turn LED on soft if a servo is attached (normal on if onboard LED is used)
    ...
    //If it's not static we can use the program button
    #if !defined(ONE_BUTTON) || !defined(STATIC_SERVO)
    ...
    //If static, use the programButton to go to mid position
    #else
    programButton.update();
    if(programButton.read() == LOW){
        gotoMid();
